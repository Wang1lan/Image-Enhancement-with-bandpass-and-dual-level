...
% -------------------------------------------------------------------------
    ...
    fineCore = dilate5x5(hardMask, 1);
    softMaskFine = buildSoftMaskFromCore(fineCore, 1);

    %% Step 3: 生成 Mid Soft Mask
    % 保护范围较大：1 次 3x3 膨胀 + 1次5×5 膨胀 + 2 次 5x5 Gaussian
    midCore_temp = dilate3x3(hardMask, 1);
    midCore = dilate5x5(midCore_temp, 1);
...
function outMask = dilate5x5(inMask, iter)
%DILATE5X5 采用 5x5 邻域的显式膨胀
%
% 输入：
%   inMask : logical 二值图
%   iter   : 膨胀次数，正整数
